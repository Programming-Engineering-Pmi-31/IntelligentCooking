...
const RecipeCard = React.memo(
    ({ getRecipe, match, dishes, setExactRecipeEmpty, setExactRecipe, rateDish, isAuth,dishesRating }) => {
        const {
            id,
            ratesCount,
            name,
        ...
        useEffect(() => {
            ...
            if (!dishes.loadedDishes[match.params.id]) {
                getRecipe(match.params.id);
            } else {
                setExactRecipe(dishes.loadedDishes[match.params.id]);
            }
            return () => {
                setExactRecipeEmpty();
            };
                                        ...
                                        <StarRatings
                                            changeRating={e => {
                                                ...
                                                if (!isAuth) {
                                                    ...
                                                    rateDish({ id: id, rating: e, count: ratesCount, totalRating: dishesRating[id] ? dishesRating[id] : rating});
                                                }
                                            }}
                                            starRatedColor="rgb(230, 67, 47)"
                                            starHoverColor="rgb(0,0,200)"
                                            rating={dishesRating[id] ? dishesRating[id] : rating}
                                            starDimension="20px"
                                            starSpacing="3px"
                                        />
                                    </span>
